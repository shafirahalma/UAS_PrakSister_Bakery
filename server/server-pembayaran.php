...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($aksi == 'tambah') {
        ...
        // $abc->tambah_data_pembayaran($data2);
    } elseif ($aksi == 'ubah') {
        $data2 = array(
            'id' => $id,
            'bahan' => $bahan,
        ...
        // $abc->ubah_data_pembayaran($data2);
    } elseif ($aksi == 'hapus') {
        // $abc->hapus_data_pembayaran($id);
    }
    unset($postdata, $data, $data2, $id, $nama_bank, $alamat, $telepon, $aksi, $abc);
} elseif ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (($_GET['aksi'] == 'tampil') and (isset($_GET['id']))) {
        $id = filter($_GET['id']);
        // $data = $abc->tampil_data_pembayaran($id);
        echo json_encode($data);
    } else {
        $data = $abc->tampil_semua_data_pembayaran();
        echo json_encode($data);
    }
